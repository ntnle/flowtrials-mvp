<script>
  ...
  import { createParticipationRequest } from '$lib/supabase';
  ...
  $: {
    if ($user && typeof window !== 'undefined') {
      ...
      if (urlParams.get('fromSignup') === 'true') {
  ...
  function handleRequestParticipate() {
    ...
    if (!$user) {
      ...
      push(`/signup?redirect=${encodeURIComponent($location)}`);
      return;
    }
  ...
  async function handleConfirmParticipation() {
    ...
    try {
      ...
      await createParticipationRequest(
        parseInt(params.id), // Convert string to integer
        participationNote,
        'email'
      );
      ...
      push('/profile?participationSuccess=true');
    } catch (err) {
...
<!-- Confirmation Modal -->
{#if showConfirmModal}
  <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4" on:click={handleCancelModal}>
